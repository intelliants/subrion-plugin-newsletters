...
$iaNewsletter = $iaCore->factoryModule('newsletter', IA_CURRENT_MODULE);
...
if (iaView::REQUEST_JSON == $iaView->getRequestType()) {
    if (isset($_GET['subscriber_email'])) {
        ...
        if (!iaValidate::isEmail($email)) {
            ...
            $iaMailer->loadTemplate('newsletter_subscription');
            $iaMailer->setReplacements([
                'newslettersToken' => $data['token'],
                'newslettersEmail' => $data['email']
            ]);
